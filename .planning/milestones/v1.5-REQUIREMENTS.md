# Requirements Archive: v1.5 Template hardening & doc parity
...
- [x] **TMPL-03** — **Confusion matrix visualization** for supervised regime classifiers: **`plot_regime_confusion_matrix()`** in **`plotting.py`**; data from **`outputs/reports/model_metrics/confusion_matrices.parquet`**; **`run_pipeline.py --steps 5 --plots`** and **`pipelines/05_predict.py --plots`**; tests in **`tests/unit/test_plot_confusion_matrix.py`**.
